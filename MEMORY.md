...
>
> **展示方式：** 你来一个想法 → 元神帮你搭建工作流 → 你能跑起来 → 持续迭代
>
> **目标用户：** 普通用户/新手用户
> **核心卖点：** 只要使用元神系统，就能快速根据自己的使用场景，搭建属于自己的工作流，稳定且质量有保障
>
> **差异化优势：**
> - 其他框架：固定框架，局限性大
> - 元神系统：可根据使用场景，搭建多条不同类型的工作流，灵活度高

**最新进展：** 
- 2026-04-26 music_generate成功生成！music-2.6可用，但文件超5MB会失败，durationSeconds建议≤30秒
- 2026-04-25 预防系统全面修复并验证通过
  - 创建 prevention-hooks.js 统一入口
  - 修复 lesson-querier 文件匹配规则（支持 *-failure.md 等格式）
  - 修复 lesson 文件格式（影响范围需要在同一行）
  - 从记忆文件提取9个重要教训到 brain/lessons/
  - AGENTS.md 添加强制规则：任务前后必须调用预防钩子
  - 测试4个场景全部通过，规避规则正确应用
- 2026-04-17 v1.3.0上传GitHub（Agent工厂+人格设定+P0/P1/P2全优化）
...
| music-2.6 | 音乐生成 | 100次/月 | ← 用户主模型 ✅ |
- ⚠️ 文件超5MB会失败，durationSeconds建议30秒内 | ← 用户主模型 ✅ |
...
- 系统状态：新增响应时间监控
- 性能监控：新增错误率趋势
- 新增健康监控增强（Token消耗/响应时间/错误率/异常告警）
- 新增效率优化（智能跳过：连续3次无变化则跳过）
- 共11项检查任务

### quick-demo 修复与完善

**修复内容：**
- 添加 `package.json`（Node.js依赖配置）
- 补充 README 运行命令与环境要求
- 明确标注「模拟演示」性质

**GitHub 推送：**
- Commit: `10ec967`
- 消息: "feat: 完善quick-demo - 添加package.json和详细运行说明"
...
## 2026-04-24 Playwright MCP 集成失败（重要教训）

### 事件摘要
用户要求用 Microsoft Playwright MCP 打开 Edge 浏览器访问抖音，完成实操测试。

### 失败原因
1. **目标错位** - 把「测试工具」当成「用工具完成任务」
2. **过度工程** - 创建复杂的 bridge 架构，把简单问题复杂化
3. **不会用内置工具** - 一直用 exec 运行自己写的脚本，忽略 OpenClaw 内置 Playwright 工具
4. **不读文档** - 没有研究内置工具的使用方法就盲目尝试
5. **多次失败不询问** - 反复尝试同一件事多次，不停下来问用户

### 核心教训
- **@playwright/mcp 只支持 Chrome**，不支持 Edge
- 环境变量配置无效（PLAYWRIGHT_BROWSERS_PATH 等）
- 不要尝试欺骗 MCP，它硬编码 Chrome 路径
- **先读文档，再动手；先验证最简单功能，再逐步扩展**

### 未清理文件
`skills/playwright-mcp/` 目录被 node 进程锁住，重启后可删除

### 相关教训文档
- `brain/lessons/2026-04-24-playwright-mcp-failure.md`
- `brain/lessons/2026-04-24-playwright-mcp-failure-2.md`

---

## 2026-04-23 三大 MCP 源码分析完成

### 分析成果
| MCP | 仓库 | Stars | 核心工具数 | 架构特点 |
|-----|------|-------|------------|----------|
| Playwright MCP | microsoft/playwright-mcp | 31,244 ⭐ | 21+ | browser_navigate/click/snapshot |
| GitHub MCP Server | github/github-mcp-server | 29,184 ⭐ | 80+ | Go语言，18个分类 |
| Firecrawl Web-Agent | firecrawl/web-agent | 846 ⭐ | Plan-Act Loop | Agent Core 27KB |
...
## 2026-04-25 StoryFlow项目失败（重大教训）

### 事件摘要
用户决定删除StoryFlow项目，这是一个从零开始开发的工作流系统。

### 失败原因
1. **WebSocket问题** - 端口冲突，多个进程争用5001端口
2. **调试失效** - 添加print语句但看不到输出，无法诊断
3. **API响应未检查** - 用户多次让我检查Network标签，我一直没做
4. **过度承诺** - 每次说"解决了"，但测试还是失败
5. **不调用子Agent** - 用户多次让我用阳神，我一直自己硬扛
...
| 规则 | 说明 |
|------|------|
| 子Agent调用 | 超过复杂度阈值必须spawn，不自己硬扛 |
| Skill主动调用 | 收到任务先扫描可用意图 |
| 诚实承诺 | 不确定时直接说，不编造 |
| 调试流程 | 先看API响应，每步验证 |
...
## 2026-04-29 浏览器自动化深度学习

### 学习内容
深入学习3个GitHub项目，提升浏览器自动化任务能力：

| 项目 | Stars | 核心收获 |
|------|-------|---------|
| **Lightpanda Browser** | 29,568 | 轻量级CDP服务器，16倍内存节省，9倍速度提升 |
| **Pydoll** | 6,780 | 反检测CDP + Pydantic结构化提取 + Shadow DOM支持 |
| **gsd-browser** | 207 | 63个命令 + Snapshot+Ref模式 + 15种语义意图 + 批量执行 |

### 核心创新：Snapshot + Ref 模式
- 版本化refs（@v1:e3）代替CSS选择器，更稳定
- 页面变化后重新snapshot即可

### 可借鉴模式（按优先级）
| 模式 | 优先级 | 说明 |
|------|--------|------|
| 断言验证 | P0 | assert --checks '[...]' 验证结果 |
| 批量执行 | P0 | batch --steps '[...]' 一次多条命令 |
| 语义意图 | P1 | act --intent accept_cookies |
| Snapshot+Ref | P1 | 稳定元素引用模式 |
| 结构化提取 | P2 | extract --schema |
| Daemon持久化 | P2 | 后台常驻快速响应 |
| 反检测 | P3 | human-like鼠标移动+打字 |

### 学习文件
- `brain/learning/2026-04-29-browser-automation-learning.md` - 详细笔记

### 与我们web-automation skill的关联
- playwright.connectOverCDP() 架构与gsd-browser一致
- 可增强：断言命令、批量执行、语义意图

---

*最后更新: 2026-04-29 03:40*

---

## 2026-04-26 教训记录系统重大升级 + 歌曲生成

### 事件摘要
凌晨03:11用户要求生成米津玄師风格日文歌+银魂风格海报，验证阳神系统工作流。

### 暴露的系统性问题

#### 1. 模型选择错误（重复犯错）
- **错误：** 子Agent使用了 music-2.5+ 而不是 music-2.6
- **原因：** 任务描述里没有明确指定模型
- **用户纠正：** "怎么又默认使用music-2.5+，minimaxx套餐支持音乐生成，不是告诉过你了吗"

#### 2. 教训记录机制缺陷
- **问题：** prevention系统找到0条教训
- **原因：** 教训没有记录到 brain/lessons/（只记到了 learnings/errors.json）
- **教训：** 预防系统查询 brain/lessons/，不查询 learnings/errors.json
...
### 已创建的教训文件
- `brain/lessons/2026-04-26-music-model-selection-failure.md` - 模型选择错误
- `brain/lessons/2026-04-26-user-correction-music-model.md` - 用户纠正教训
...
1. **任务描述必须包含模型配置：**
   ...
   ```
2. **教训记录规则：** 所有失败必须记录到 brain/lessons/
3. **用户纠正时：** 回应 + 记录教训 + 立即修正
