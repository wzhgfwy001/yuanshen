...
**触发场景：**
- 用户说"自动抓取这个网站的数据" → 自动调用此Skill执行爬虫+浏览器自动化
- 用户说"帮我填表提交"、"批量发布内容" → 自动表单填写+提交
- 用户说"定时监控XX网站"、"每天早上查一下" → 自动创建定时任务
- 用户说"需要登录后才能操作" → 自动状态管理+多账号切换
- 复杂网页交互（点击/滚动/悬停）→ 自动CDP连接+元素操作

**示例对话：**
- 用户："帮我监控某商品的 价格，每天早上8点发消息给我"
- AI自动使用此Skill → 创建定时任务 → 每日抓取 → 推送结果

- 用户："批量发布文章到10个平台"
- AI自动使用此Skill → 循环登录各平台 → 批量填写提交

- 用户："帮我登录这个后台，截图看看后台界面"
- AI自动使用此Skill → 保存登录状态 → 导航 → 全页截图 → 分析界面

**不适用场景：**
- 简单静态页面不需要此Skill（用 `web_fetch` 即可）
- 纯API调用不需要启动浏览器
- 单次快速查询优先用 `web-automation` 而非此套件
