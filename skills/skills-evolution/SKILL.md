...
**触发场景：**
- 某类任务连续完成5次以上，成功率>80% → 自动提示创建新Skill
- 某个Skill被调用后成功率<60% → 自动触发改进审查
- 系统自动追踪任务完成情况，无需用户手动触发

**示例对话：**
- 系统检测到"代码审查"任务连续成功5次，成功率90%
- 系统提示："检测到代码审查任务模式，可自动创建Skill草稿，是否确认？"
- 用户确认 → 自动生成Skill草稿到 `skills/skill-drafts/`

**不适用场景：**
- 单次任务不需要此Skill
- 失败率高的任务（成功率<80%）→ 需先优化流程
- 非重复性的一次性任务
