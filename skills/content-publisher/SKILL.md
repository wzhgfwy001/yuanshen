...
**触发场景：**
- 用户说「发布内容」「保存内容」「导出内容」「publish」「输出」时自动调用
- 用户要求将内容保存到本地文件或内网系统时自动调用

**示例对话：**
- 用户：「帮我保存这篇文章到桌面」
- AI自动使用此Skill处理，生成文件并保存
- 用户：「导出为HTML格式」
- AI自动调用此Skill生成指定格式文件

**不适用场景：**
- 简单问答不需要此Skill（如「什么是发布」）
- 纯写作不需要（用 writing-blog）
- 只需改写风格不需要（用 xiaohongshu-editor）
