...
**触发场景：**
- 用户说"分析数据"、"data analysis"、"数据分析"、"生成报表"、"统计" → 自动调用此Skill
- 用户要求"帮我看看这份数据"、"生成可视化图表"
- 用户发送数据文件（Excel/CSV/PDF）并要求分析

**示例对话：**
- 用户："帮我分析这份销售数据"
- AI自动使用此Skill处理，执行描述性统计和模式识别

- 用户："生成本月的运营报表"
- AI自动调用数据分析模块，生成Markdown格式报告

- 用户：[发送CSV文件] "这个数据有什么规律？"
- AI自动读取文件并执行 data-analysis-assistant
