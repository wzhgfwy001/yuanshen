...
**触发场景：**
- 用户说"这是什么任务类型" → 自动调用此Skill分类任务
- 用户说"帮我判断这个任务用同步还是异步" → 自动分析
- 复杂任务需要决定是否启用多Agent → 自动调用typologist判断

**示例对话：**
- 用户："帮我分析这个系统架构"
- AI自动使用此Skill → 判断为 `decomposed` 类型 → 启用多Agent协作

- 用户："监控一下这个网站有没有更新"
- AI自动使用此Skill → 判断为 `monitor` 类型 → 后台持续运行

- 用户："帮我写完这个报告"
- AI自动使用此Skill → 判断为 `workflow` 类型 → 多步骤工作流处理

**不适用场景：**
- 简单问答不需要此Skill（直接 `query` 类型即可判断）
- 单一操作且无复杂度的不需要细分
