...
**触发场景：**
- 用户说"帮我写..."（小说、剧本、系列文章）→ 自动创建子Agent团队
- 用户说"分析数据"或"生成报告" → 自动启动数据分析模块
- 用户说"调研..."或"研究..." → 自动多角度信息整合
- 复杂任务（多步骤、需多角色协作）→ 自动动态分配子Agent

**示例对话：**
- 用户："写一本科幻小说，10章"
  → AI自动识别为复杂创意任务 → 创建4-6个子Agent → 剧情架构+世界观+内容创作+审查
- 用户："分析这份销售数据，生成报告"
  → AI自动启动数据分析模块 → 子Agent处理Excel/CSV → 汇总生成报告
- 用户："帮我做这三件事"
  → AI自动识别为可并行任务 → 并行创建子Agent → 同时执行

**不适用场景：**
- 简单查询（"今天天气如何"）不需要此Skill
- 单一计算任务直接完成即可
- 已有专用Skill的标准任务（如代码审查用code-review-assistant）
- 强实时性要求的任务（<1秒响应）
