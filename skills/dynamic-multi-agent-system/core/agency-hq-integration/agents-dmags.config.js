...
const DMAGS_ROLES = {
  // 创意写作类
  WORLDBUILDER: { id: 'worldbuilder', name: '世界观架构师', emoji: '🏰', desk: 'research' },
  OUTLINER: { id: 'outliner', name: '大纲设计师', emoji: '📐', desk: 'design' },
  CHARACTER_DESIGNER: { id: 'character_designer', name: '角色塑造师', emoji: '🎭', desk: 'design' },
  PLOT_WEAVER: { id: 'plot_weaver', name: '剧情编织师', emoji: '🕸️', desk: 'research' },
  WRITER: { id: 'writer', name: '文字炼金师', emoji: '⚗️', desk: 'content' },
  REVIEWER: { id: 'reviewer', name: '审查官', emoji: '🔍', desk: 'security' },
  
  // 数据分析类
  DATA_ANALYST: { id: 'data_analyst', name: '数据分析师', emoji: '📊', desk: 'research' },
  STRATEGIST: { id: 'strategist', name: '策略顾问', emoji: '💡', desk: 'strategy' },
  
  // 技术开发类
  ARCHITECT: { id: 'architect', name: '架构师', emoji: '🏗️', desk: 'engineering' },
  DEVELOPER: { id: 'developer', name: '开发工程师', emoji: '💻', desk: 'dev' },
  TESTER: { id: 'tester', name: '测试工程师', emoji: '🧪', desk: 'engineering' },
};
...
const ACCESSORY_MAPPING = {
  'worldbuilder': 'glasses',
  'outliner': 'monocle',
  'character_designer': 'bowtie',
  'plot_weaver': 'scarf',
  'writer': 'headphones',
  'reviewer': 'badge',
  'data_analyst': 'visor',
 ...
 */
function generateAgentConfigs(taskDecomposition, model = 'qwen3.5-plus') {
  const agents = [];
  const subtasks = taskDecomposition.subtasks || [];
  
  subtasks.forEach((subtask, index) => {
    const roleKey = getRoleFromSubtask(subtask);
    const roleConfig = DMAGS_ROLES[roleKey];
    ...
    const modelConfig = MODEL_MAPPING[model] || MODEL_MAPPING['qwen3.5-plus'];
    const accessory = ACCESSORY_MAPPING[roleKey] || 'glasses';
 ...
 * 根据子任务类型推断角色
 */
function getRoleFromSubtask(subtask) {
 ...
 * 根据任务状态更新 Agent 状态
 */
function updateAgentStatus(agent, taskState) {
  return {
    ...agent,
 ...
 * 计算 Agent 应该所在的房间
 */
function assignAgentRoom(agent, taskState) {
  // 有未完成的依赖 → 会议室
