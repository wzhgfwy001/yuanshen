...
function diagnoseError(
  ...
  // API限流：根据配额判断
  if (errorCode === 'ERR-API-0204' || errorCode === 'ERR-NET-0114') {
    const quotaReset = context.metadata?.quotaResetTime;
    if (quotaReset && Date.now() > quotaReset) {
      action = 'RETRY';
    } else {
      action = 'SKIP';
    }
...
interface ErrorHandler {
  ...
  registerCustomError(code: string, definition: Partial<ErrorDefinition>): void;
  getErrorHistory(filter?: ErrorFilter): ErrorRecord[];
  getErrorStats(): ErrorStats;
}

class UnifiedErrorHandler implements ErrorHandler {
  private errorDefs: Record<string, ErrorDefinition>;
  ...
  async handle(error: Error, context: ErrorContext): Promise<ErrorResult> {
    ...
    // 2. 获取错误定义
    const definition = this.errorDefs[errorCode] || this.errorDefs['ERR-SYS-0007'];
    
    // 3. 自动诊断
    const result = diagnoseError(errorCode, context);
    
    // 4. 记录日志
    this.logError(errorCode, error, context, result);
    
    // 5. 执行处理
    switch (result.action) {
      case 'RETRY':
        return this.handleRetry(error, context, result);
      case 'SKIP':
        return this.handleSkip(error, context, result);
      case 'ABORT':
        return this.handleAbort(error, context, result);
    }
  }
  
  private extractErrorCode(error: Error): string {
    // 从错误信息中提取错误码
    const match = error.message.match(/ERR-[A-Z]+-\d{4}/);
    return match ? match[0] : 'ERR-SYS-0007';
  }
  ...
  private logError(
    ...
    // 根据错误级别记录日志
    const def = this.errorDefs[code];
    console[def?.logLevel || 'error'](`[${code}] ${error.message}`, {
      context,
      result
    });
  }
...
const USER_MESSAGES: Record<string, string> = {
  ...
  'ERR-SYS-0007': '遇到了一个意外问题，我们已记录并会改进',
  ...
  // 通用
  'ERR-SYS-0007': '遇到了一点问题，请稍后重试',
};

// 生成用户友好的错误消息
function getUserMessage(errorCode: string, context?: Record<string, string>): string {
  let message = USER_MESSAGES[errorCode] || USER_MESSAGES['ERR-SYS-0007'];
...
// 获取错误统计
function getErrorStats(filter?: {
  startTime?: number;
  endTime?: number;
  category?: ErrorCategory;
}): ErrorStats {
  const history = errorHandler.getErrorHistory(filter);
  
  const stats: ErrorStats = {
    totalErrors: history.length,
    byCategory: { RETRY: 0, SKIP: 0, ABORT: 0 },
    byCode: {},
    topErrors: [],
    retryRate: 0,
    skipRate: 0,
  ...
  history.forEach(record => {
    stats.byCategory[record.result.action]++;
    stats.byCode[record.code] = (stats.byCode[record.code] || 0) + 1;
  });
  ...
  if (stats.totalErrors > 0) {
    stats.retryRate = stats.byCategory.RETRY / stats.totalErrors;
    stats.skipRate = stats.byCategory.SKIP / stats.totalErrors;
    stats.abortRate = stats.byCategory.ABORT / stats.totalErrors;
  }
...
│  ├─ ERR-NET-0114：42次（限流）
│  ├─ ERR-AGT-0303：28次（超时）
│  ├─ ERR-API-0206：21次（服务错误）
│  ├─ ERR-CACHE-0802：18次（缓存过期）
│  └─ ERR-TIME-1101：12次（请求超时）
│
├─ 恢复率：90%
└─ 平均恢复时间：2.3秒
```
