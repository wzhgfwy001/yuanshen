...
**触发场景：**
- 用户说"记住我的偏好" → 自动调用此Skill更新用户画像
- 用户说"我之前是怎么处理的" → 自动检索画像中的偏好信息
- 用户纠正AI行为时 → 自动调用此Skill记录纠正内容到画像
- 新session开始时 → 自动加载用户画像辅助理解用户

**示例对话：**
- 用户："我喜欢简洁的回复，不要废话"
- AI自动使用此Skill → 更新 `preferences.communication_style = "简洁"`

- 用户："以后这类问题用表格展示"
- AI自动使用此Skill → 更新 `preferences.output_format = "表格"`

- 用户："纠正一下，上次你说错了，我偏好分步处理而不是一口气说完"
- AI自动使用此Skill → 记录 `feedback_history` 并更新 `decision_style`

**不适用场景：**
- 纯闲聊无信息量时不需要记录
- 用户明确表示"一次性偏好，不用记住"时跳过
