...
// From yangshen-flow.js
const __dirname = 'C:/Users/DELL/.openclaw/workspace/skills/dynamic-multi-agent-system/core/flow';
...
// Now test the flow's matchAgents logic
...
// Fallback logic from yangshen-flow.js
    ...
    for (const [keyword, agentNames] of Object.entries(CATEGORY_KEYWORD_MAP)) {
        if (taskLower.includes(keyword)) {
            ...
            for (const agentName of agentNames) {
                ...
                if (agent && !matches.find(m => m.id === agent.id)) {
                    console.log('  Adding fallback agent:', agent.name_zh || agent.name);
