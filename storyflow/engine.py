...
class CacheManager:
    ...
    def _make_cache_key(self, node_id: str, inputs: Dict[str, Any]) -> str:
        """生成缓存键"""
        content = f"{node_id}:{json.dumps(inputs, sort_keys=True)}"
        return hashlib.sha256(content.encode()).hexdigest()[:16]
    
    def get(self, node_id: str, inputs: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """获取缓存结果"""
        cache_key = self._make_cache_key(node_id, inputs)
    ...
    def set(self, node_id: str, inputs: Dict[str, Any], outputs: Dict[str, Any]):
        """保存缓存结果"""
        cache_key = self._make_cache_key(node_id, inputs)
        cache_file = os.path.join(self.cache_dir, f"{cache_key}.json")
...
class Workflow:
    ...
    def add_connection(self, source_node: str, source_output: str, target_node: str, target_input: str):
        """添加连接"""
        conn_key = f"{source_node}:{source_output}->{target_node}:{target_input}"
...
class LoopEngine(Engine):
    ...
    async def _execute_loop(self, initial_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        while self.iteration_count < self.loop_config.max_iterations:
            ...
            print(f"\n🔄 循环迭代 {self.iteration_count}/{self.loop_config.max_iterations}")
    ...
    async def _execute_loop_nodes(self) -> Dict[str, Any]:
        ...
        for node_id in self.loop_config.loop_nodes:
            ...
            try:
                ...
                print(f"❌ 循环节点 {node.name} 执行失败: {str(e)}")
                self.execution_log.append({
                    "node_id": node_id,
                    "status": "error",
                    "error": str(e),
