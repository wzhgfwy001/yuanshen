...
import os
from engine import Node, LLMNode, NodeResult, ProviderFactory


def get_api_key():
    """从环境变量获取API Key"""
    # 优先使用 STORYFLOW_API_KEY
    api_key = os.environ.get("STORYFLOW_API_KEY")
    if not api_key:
        # 尝试通义千问
        api_key = os.environ.get("DASHSCOPE_API_KEY") or os.environ.get("OPENAI_API_KEY")
    if not api_key:
        raise ValueError("请设置环境变量 STORYFLOW_API_KEY 或 DASHSCOPE_API_KEY")
    return api_key
