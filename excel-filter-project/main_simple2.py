...
class App:
    ...
    def _create_ui(self):
        ...
        tk.Label(title_frame, text="山东高考志愿填报系统 - 本科", font=("微软雅黑", 16, "bold"), bg='#1565c0', fg='white', pady=8).pack()
        tk.Label(title_frame, text="✦ 硕博教育咨询师专用 ✦", font=("微软雅黑", 9), bg='#1565c0', fg='#90caf9', pady=3).pack()
        ...
        tk.Label(left, text="📊 分数输入", font=("微软雅黑", 11, "bold"), bg='#f8f9fa', fg='#1565c0').pack(anchor='w')
        ...
        for subj in ["语文", "数学", "英语"]:
            ...
            tk.Entry(f, textvariable=v, width=5, font=("微软雅黑", 10), justify='center', bg='#e8f5e9', relief='flat').pack(side='left', padx=2)
        ...
        self.total_l = tk.Label(t_f, text="总分: 0", font=("微软雅黑", 14, "bold"), bg='#1565c0', fg='white', anchor='center')
        ...
        tk.Label(left, text="🔍 筛选条件", font=("微软雅黑", 11, "bold"), bg='#f8f9fa', fg='#1565c0').pack(anchor='w', pady=6)
        ...
        tk.Label(frm, text="层次:", font=("微软雅黑", 9), bg='#f8f9fa').grid(row=1, column=0, sticky='nw', pady=2)
        lf = tk.Frame(frm, bg='#f8f9fa')
        lf.grid(row=1, column=1, sticky='w', pady=2, padx=4)
        self.lv = {}
        for i, l in enumerate(["985", "211", "双一流", "本科"]):
            var = tk.BooleanVar(value=False)
            self.lv[l] = var
            tk.Checkbutton(lf, text=l, variable=var, font=("微软雅黑", 8), bg='#f8f9fa').grid(row=0, column=i, sticky='w', padx=2)
        
        tk.Label(frm, text="学校:", font=("微软雅黑", 9), bg='#f8f9fa').grid(row=2, column=0, sticky='nw', pady=2)
        self.sv = tk.StringVar()
        tk.Entry(frm, textvariable=self.sv, width=18, font=("微软雅黑", 9)).grid(row=2, column=1, sticky='w', pady=2, padx=4)
        
        tk.Label(frm, text="专业:", font=("微软雅黑", 9), bg='#f8f9fa').grid(row=3, column=0, sticky='nw', pady=2)
        self.mv = tk.StringVar()
        tk.Entry(frm, textvariable=self.mv, width=18, font=("微软雅黑", 9)).grid(row=3, column=1, sticky='w', pady=2, padx=4)
        
        bf = tk.Frame(left, bg='white')
        bf.pack(fill='x', pady=6)
        tk.Button(bf, text="开始筛选", font=("微软雅黑", 10, "bold"), bg='#1565c0', fg='white', pady=4, command=self._filter).pack(fill='x', pady=1)
        ...
        self.stats_l = tk.Label(right, text="等待筛选...", font=("微软雅黑", 12), bg='white', fg='#1565c0', pady=6)
        ...
        self.next = tk.Button(pf, text="下一页", font=("微软雅黑", 9), command=self._next, state='disabled', bg='#1565c0', fg='white')
    ...
    def _load_data(self):
        ...
        def load():
            try:
                ...
                if not files: messagebox.showerror("错误", "未找到Excel!"); return
                self.df = pd.read_excel(os.path.join(ws, files[0]), sheet_name=0)
                self.df.rename(columns={'院校名称':'院校名称','25专业名称':'专业名称','25年预测分数':'预测分数','省份':'省份','城市':'城市','院校水平':'院校层次'}, inplace=True)
                self.st.config(text=f"加载完成: {len(self.df)} 条")
                messagebox.showinfo("成功", f"加载完成\n{len(self.df)} 条")
            except Exception as e:
                self.st.config(text=f"失败: {e}")
    ...
    def _filter(self):
        ...
        try:
            ...
            # 层次筛选逻辑
            selected_levels = [k for k, v in self.lv.items() if v.get()]
            if selected_levels:
                if '本科' in selected_levels:
                    # 选"本科"时，显示所有本科（含985/211/双一流），排除非本科
                    df = df[~df['院校层次'].fillna('').str.contains(r'专科|3\+3|高职|技师|技工', regex=True)]
                else:
                    # 没选本科时，只显示选中层次的院校
                    df = df[df['院校层次'].fillna('').str.contains('|'.join(selected_levels), regex=True)]
            ...
            if selected_subjects:
                ...
                if req_col in df.columns:
                    pattern = '|'.join(selected_subjects)  # 用|连接实现OR
                    df = df[df[req_col].fillna('').str.contains(pattern, regex=True)]
            else:
                # 选"不限"：显示无专业要求的
                req_col = '25选科要求'
                if req_col in df.columns:
                    if self.subject_vars["不限"].get():
                        df = df[df[req_col].isna() | (df[req_col].str.strip() == '') | (df[req_col] == '不限')]
            
            results = {'冲刺': [], '稳妥': [], '保底': []}
            for _, r in df.iterrows():
                p = r.get('预测分数', 0)
                if pd.isna(p) or p == 0: continue
                d = total - p  # d > 0 表示高于预测
                
                # 新概率算法：d = 总分 - 预测分数
                if d < -30:  # 低于预测30分以上，不计入
                    continue
                elif d < -10:  # 低于预测10-30分，冲刺（概率30-50%）
                    prob = 30 + (30 + d) * (20/20)  # d=-30→30%, d=-10→50%
                ...
                elif d < 10:  # 在预测±10分之间，稳妥（概率50-70%）
                    if d <= 0:
                        prob = 50 + (d + 10) * (20/10)  # d=-10→50%, d=0→70%
                    else:
                        prob = 70 - d * (20/10)  # d=0→70%, d=10→50%
                ...
                elif d <= 25:  # 高于预测10-25分，保底（概率70-100%）
                    prob = 70 + (d - 10) * (30/15)  # d=10→70%, d=25→100%
            ...
            # 按顺序分配数量：冲刺30、稳妥50、保底20
            for cat, target in [('冲刺', 30), ('稳妥', 50), ('保底', 20)]:
                school_count = {}  # 每个分类独立计数
                count = 0
    ...
    def _show_page(self):
        ...
        self.prev.config(state='normal' if self.current_page>1 else 'disabled', bg='#9e9e9e' if self.current_page<=1 else '#1565c0')
        self.next.config(state='normal' if self.current_page<tp else 'disabled', bg='#9e9e9e' if self.current_page>=tp else '#1565c0')
