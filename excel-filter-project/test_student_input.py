...
class StudentInputDialog(ctk.CTkToplevel):
    ...
    def __init__(self, parent, title: str = "学生信息输入"):
...
姓名：{name}
选科：{', '.join(selected_subjects)}
总分：{total}
排名：{self.rank_entry.get() or '未填写'}
...
if __name__ == "__main__":
    ...
    app.title("Excel 筛选器 - 测试")
    
    def open_dialog():
        dialog = StudentInputDialog(app)
        dialog.grab_set()
    
    btn = ctk.CTkButton(app, text="打开学生输入对话框", command=open_dialog, height=50)
    btn.pack(pady=50)
    
    # 说明
    info = ctk.CTkLabel(app, text="功能说明：\n1. 手动输入学生信息\n2. 勾选选科科目\n3. 输入各科分数\n4. 自动计算总分\n5. 保存后开始筛选", justify="left")
    info.pack(pady=10)
